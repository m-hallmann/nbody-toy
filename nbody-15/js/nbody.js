...
function mergeBodies(i, j) {
    ...
    if (pBody.mass > aBody.mass) {
        ...
        // replaced dist with deltaXYtoStr
        // pBody.iStr = dist(aBody.x, aBody.y, aBody.x + sumDX, aBody.y + sumDY);
        aBody.iStr = deltaXYtoStr(sumDX, sumDY);
        // same
        // aBody.iAng = Math.PI * .5 - radian(aBody.x, aBody.y, aBody.x + sumDX, aBody.y + sumDY);
        aBody.iAng = Math.PI * .5 - deltaXYtoRadian(sumDX, sumDY);
...
function update() {
    ...
    for (var i = 0; i < n; i++) {
                ...
                if (aBody.untouchable == 0) {
                    ...
                    if ((i != j) && powerSaving(thisDistance, n)) {
                        ...
                        if (thisDistance >= (pBody.radius + aBody.radius) ) {
                            ...
                            aStr = deltaXYtoStr(deltaX(pBody.iAng, pBody.iStr) + attractionX, deltaY(pBody.iAng, pBody.iStr) + attractionY);
                            ...
                            aAng = Math.PI * .5 - deltaXYtoRadian(deltaX(pBody.iAng, pBody.iStr) + attractionX, deltaY(pBody.iAng, pBody.iStr) + attractionY);
        ...
        pBody.sphere.scale.x = pBody.radius;
        pBody.sphere.material.color.setHSL(pBody.iStr/(pBody.mass*INERTIA), pBody.iStr*60/100, (pBody.mass/4+40)/100);
