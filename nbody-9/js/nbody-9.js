 ...
 *   
 *   - Fix Collision bug
 *     Collisions use wrong equation to conserve impulse. Currently,
 *     Impulse = Speed, which is wrong. Need to separate values!
 ...
 *   - Zoom
 *      - Mousewheel zoom
 *        Zoom should be an expression in the draw sequence.
 *        That way, only the projection on the canvas is affected.
 *        This requires:
 *         - Zoom multiplier
 ...
 *   - Spawn on MouseDown
 *      - Random emission of small particles on mousedown
...
// Constants
var N_INITIAL = 650;     // Number of bodies
...
var CLR = 30;            // clear constant; every CLRth iteration, the screen is cleared with opaque black.
...
function drawBody(which) {
    ...
    if (thisBody) {
        ...
        // Body
        ctx1.beginPath();
        ctx1.arc(thisBody.x * scale + offsetX, thisBody.y * scale + offsetY, radius(thisBody.mass) * scale,0,2*Math.PI);
...
function update() {
    ...
    for (var i = 0; i < n; i++) {
        ...
        // randomly remove current body from array. tests the draw function, which will throw an error if it tries to paint a removed object.
//        if (Math.random()*100 <= 1) {
//            nbody = nbody.filter(c=>c!==nbody[passiveBody]);
//            n--;
//        }
        ...
        for (var j = 0; j < n; j++) {
            ...
            if (i != j) {
                ...
                if (thisDistance >= (radius(pBody.mass) + radius(aBody.mass))) {
