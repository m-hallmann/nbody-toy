...
var CLR = 10;            // clear constant; every CLRth iteration, the screen is cleared with opaque black.
...
function drawBody(which) {
    ...
    if (thisBody) {
        color = 'hsl(50,'+thisBody.iStr+'%, '+thisBody.iStr+'%)';
        color2 = color;
        ...
        if (thisBody.type==3) {
            ...
            // Tracer
            ctx1.beginPath();
            ctx1.arc(thisBody.x * scale + offsetX,thisBody.y * scale + offsetY,.1,0,2*Math.PI);
...
/*
        if (pBody.x >= viewW/2) {
            pBody.x = 0-viewW/2+50;
            pBody.iStr = .5;
        } else if (pBody.x <= 0 - viewW/2) {
            pBody.x = viewW/2-50;
            pBody.iStr = .5;
        }

        if (pBody.y >= viewH/2) {
            pBody.y = 0-viewH/2+50;
            pBody.iStr = .5;
        } else if (pBody.y <= 0 - viewH/2) {
            pBody.y = viewH/2-50;
            pBody.iStr *= .5;
        }
*/
