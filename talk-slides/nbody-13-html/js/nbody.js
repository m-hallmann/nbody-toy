...
var viewH = window.innerHeight * 2;          // viewport height, not updated!
var offsetX = viewW/2;
var offsetY = viewH/2;
var scale = 1;
...
var canvasD = document.getElementById("canvasDiv");
canvasD.width  = viewW;
canvasD.height = viewH;
...
function p(nbody){
  ...
  for (var i = 0; i < nbody.length; i++) {
    canvasD.insertAdjacentHTML('afterbegin', '<div class="p" id="p-'+i+'" style="top:'+nbody[i].y+'; left:'+nbody[i].x+'; width:'+nbody[i].mass+'; height:'+nbody[i].mass+'"></div>');
  }
}
p(nbody);


function pUpdate(nbody){

  for (var i = 0; i < nbody.length; i++) {
    // console.log(i);

    var par = document.getElementById('p-'+i);
    par.style.top = nbody[i].y;
    par.style.left = nbody[i].x;
    par.style.width = nbody[i].mass;
    par.style.height = nbody[i].mass;
  }
